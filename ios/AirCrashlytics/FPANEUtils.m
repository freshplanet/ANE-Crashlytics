...
{
    int32_t intValue;
    FREGetObjectAsInt32(object, &intValue);
...
{
    double doubleValue;
    FREGetObjectAsDouble(object, &doubleValue);
    return doubleValue;
}
...
NSArray * FPANE_FREObjectToNSArrayOfNSString(FREObject object)
{
    ...
    {
        FREObject itemRaw;
        FREGetArrayElementAt(object, (uint32_t)i, &itemRaw);
...
NSDictionary * FPANE_FREObjectsToNSDictionaryOfNSString(FREObject keys, FREObject values)
{
    ...
    for (NSInteger i = 0; i < numItems; i++)
    {
        ...
        FREGetArrayElementAt(keys, (uint32_t)i, &keyRaw);
        FREGetArrayElementAt(values, (uint32_t)i, &valueRaw);
